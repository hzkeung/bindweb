...
class IndexHandler(BaseHandler):
    ...
    def get_dnsmasq(self):
        status = os.system("ps aux | grep -v grep |grep bind -q")
        v1 = os.popen("/usr/local/bind/sbin/named -v|awk '{print $2}'")
        version = v1.read().strip()
        return {"version":version, "status":status}

    def get_cpu(self):
        cpu = os.popen('top -bi -n 1').read().split('\n')[2]
        cpu = cpu.split(", ")[3].split('%')[0].split(" ")[0]
        return 100.0 - float(cpu)

    def get_zone_count(self):
        zone_count = self.db.get('''select count(id) as count from t_zones''')
        return zone_count['count']
    def get_record_count(self):
        record_count = self.db.get('''select count(id) as count from t_records''')
        return record_count['count']
    def get_user_count(self):
        user_count = self.db.get('''select count(id) as count from t_users''')
    ...
    def get(self):
        ...
        self.render2("index.html",dashboard="active",data=data,count=count)
