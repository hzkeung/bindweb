...
class LoginHandler(BaseHandler):
    ...
    def post(self,*args):
        ...
        user = self.db.get('''select id,username,status from t_users where username = %s and password = md5(%s)''',username,password)
        if user:
            if user['status'] == 'no':
        ...
        # 写登录日志
        self.db.execute(''' insert into t_login_logs (uid,username,login_host,user_agent) values (%s,%s,%s,%s) ''',user['id'],user['username'],login_host,user_agent)
        # 登录成功，客户端写cooike
        if remember == 'yes':
            expires = 30
        else:
            expires = None
        self.set_secure_cookie('auth_token',username,expires_days=expires)
        # 跳转到登录前的页面
