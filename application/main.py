...
class BaseHandler(RequestHandler, TemplateRendering):
    ...
    def set_default_headers(self):
        ...
        self.set_header("X-Powered-By","nginx")
    ...
    def get_current_user(self):
        username = self.get_secure_cookie("auth_token")
        if not username: return None
        #return self.db.get("SELECT * FROM t_users WHERE id = %s", int(user_id))
        return username

    #@property
    def user_info(self):
        if self.current_user:
            user = self.db.get("SELECT id,username,name FROM t_users WHERE username = %s", self.current_user)
            return user
        else:
            return None
